...
K-system meter (display-only). K-12 / K-14 / K-20 modes + streaming target overlays + group aggregate via SharedWorld.
...
BWA-KMeter is a standalone K-system display. K-12 / K-14 / K-20 modes. Streaming target overlays. Group-level aggregate option via SharedWorld — show LUFS for the drum group, for example.
...
BWA-KMeter ships on **two surfaces** — same DSP, different UI:
...
[Releases](https://github.com/vmcguire/BWA-KMeter/releases). Build from
