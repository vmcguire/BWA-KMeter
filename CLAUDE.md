# BWA-KMeter — Read this before doing anything
...
can clone, browse, and reason about BWA-KMeter on its own without
seeing the rest of the BWA monorepo.

All real work on BWA-KMeter happens inside the BWA monorepo at:

```
~/Developer/bwa-fx-ecosystem/plugins/kmeter/
    dsp/           ← authoritative DSP implementation (THE source of truth)
    ui/            ← UI surface (Metal-rendered)
    plugin/        ← VST3 wrapper → builds BWA-KMeter.vst3
    manifest.json  ← name, controls, license tier, ecosystem benefits
```

If you've been asked to "work on BWA-KMeter," **navigate to the
...
1. **Standalone VST3** (this repo's release artifact) — `BWA-KMeter.vst3`
...
- **Role:** Post-merge K-system meter (display only).
- **Pain attacked:** Standalone K-system display + streaming overlays + group aggregate via SharedWorld.
- **Scope verdict:** GROUP/ALL (read-only) — 'Show LUFS for the drum group'
...
4. **[`BWA-Architecture/docs/MARKET-PAIN-AND-ATTACKS.md`](https://github.com/vmcguire/BWA-Architecture/blob/main/docs/MARKET-PAIN-AND-ATTACKS.md)**
   — the row for BWA-KMeter. Internalize the pain + the attack before
...
- ❌ Committing to this repo. Go to the monorepo (`plugins/kmeter/`).
...
- ❌ Adding new shared/* modules without checking if the capability folds
  ...
  Methodology Rule 3."
- ❌ Skipping the BWA-KMeter row in MARKET-PAIN-AND-ATTACKS.md. If the
  proposed work doesn't advance the BWA Attack for this plugin, push back.

## What "done" looks like

A `BWA-KMeter.vst3` that:
...
```bash
git subtree push --prefix=plugins/kmeter \
  git@github.com:vmcguire/BWA-KMeter.git main
```

Then on GitHub:

1. Create a Release from `main` in this repo.
2. Attach the notarized `BWA-KMeter.vst3` zip.
3. Reference the monorepo commit SHA in the release notes.

---

_End of LLM onboarding for BWA-KMeter._
